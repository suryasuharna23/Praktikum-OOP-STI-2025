...
public abstract class Boss {
    ...
    public static void playPantheon(Knight knight) {
        ...
        int mod = 0;
        if (difficulty.equals("Attuned")) mod = 0;
        else if (difficulty.equals("Ascended")) mod = 20;
        else if (difficulty.equals("Radiant")) mod = 30;

        for (int i = 0 ; i< pantheonSize; i++) {
            if (pantheon[i] instanceof SpecialBosses) {
                pantheon[i].setAttackPower(pantheon[i].getAttackPower() + mod);
                ((SpecialBosses)pantheon[i]).specialAttack(knight);
            }
            pantheon[i].setAttackPower(pantheon[i].getAttackPower());
        }
